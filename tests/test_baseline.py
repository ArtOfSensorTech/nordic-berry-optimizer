...
class BaselineTests(unittest.TestCase):
    def test_prompt_matches_frozen_template_with_water(self):
        case = {"sliders": {"data": 100, "genius": 0, "fit": 0, "cute": 0}, "power_mode": False, "stimulant_boost": False, "liquid_base": "water"}
        self.assertEqual(baseline_prompt(case), FROZEN_PROMPT.format(data=100, genius=0, fit=0, cute=0, power_mode="off", stimulant_boost="off", liquid_base="water"))

    def test_prompt_includes_mineral_water(self):
        case = {"sliders": {"data": 0, "genius": 100, "fit": 0, "cute": 0}, "power_mode": False, "stimulant_boost": False, "liquid_base": "mineral_water"}
        self.assertEqual(baseline_prompt(case), FROZEN_PROMPT.format(data=0, genius=100, fit=0, cute=0, power_mode="off", stimulant_boost="off", liquid_base="mineral_water"))

    def test_prompt_rejects_invalid_liquid_base(self):
        case = {"sliders": {"data": 0, "genius": 0, "fit": 0, "cute": 0}, "liquid_base": "mixed_water"}
        with self.assertRaisesRegex(ValueError, "liquid_base"):
